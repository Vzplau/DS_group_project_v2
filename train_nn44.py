"""
OPTION 4: THE OPTUNA CHALLENGER (NO LEAKAGE VERSION)
STRICT DATA DISCIPLINE: Train / Validation / Test
"""
...
print("NO DATA LEAKAGE - STRICT TRAIN/VAL/TEST SPLIT")
print("="*70)
...
# ============================================================================
# LOAD DATA WITH PROPER 3-WAY SPLIT
# ============================================================================
...
# ============================================================================
# CRITICAL: 3-WAY SPLIT - TRAIN / VALIDATION / TEST
# ============================================================================
...
# This gives us: 60% train, 20% val, 20% test of original data
...
print("\n" + "="*70)
print("DATA USAGE RULES")
print("="*70)
print("✓ TRAIN:      Used for model training (gradient updates)")
print("✓ VALIDATION: Used ONLY for hyperparameter selection (Optuna)")
print("✗ TEST:       LOCKED until final evaluation - NEVER touched!")
print("="*70)
...
print(f"\nPos weight (from train only): {pos_weight:.3f}")
...
def train_model(model, train_loader, val_loader, config, device='cuda'):
    """
    Train model on TRAIN set, validate on VALIDATION set
...
# ============================================================================
# OPTUNA OBJECTIVE - NO TEST SET LEAKAGE!
# ============================================================================

def objective(trial):
    """
    Optuna optimization using ONLY train and validation sets
    ...
    # Create data loaders - TRAIN and VALIDATION only!
    ...
    # Train model (uses train + val, NO TEST!)
    try:
        trained_model, val_f1 = train_model(model, train_loader, val_loader, config, device)
...
print("Optimizing on VALIDATION set only - Test set remains locked!")
print("="*70)
...
# ============================================================================
# TRAIN FINAL MODEL ON TRAIN+VAL (optional strategy)
# ============================================================================

print("\n" + "="*70)
print("TRAINING FINAL MODEL")
print("="*70)
print("Strategy: Retrain on TRAIN+VAL with best hyperparameters")
print("(This is common practice to use all available data before final test)")

best_params = study.best_params

# Combine train and val for final training
X_trainval_scaled = np.vstack([X_train_scaled, X_val_scaled])
y_trainval_combined = np.concatenate([y_train.values, y_val.values])

X_trainval_tensor = torch.FloatTensor(X_trainval_scaled)
y_trainval_tensor = torch.FloatTensor(y_trainval_combined)
...
# Create dummy val loader (won't be used for selection, just for training loop)
trainval_dataset = TensorDataset(X_trainval_tensor, y_trainval_tensor)
trainval_loader = DataLoader(trainval_dataset, batch_size=best_params['batch_size'], shuffle=True)

# Use a small portion of trainval as internal validation for early stopping
n_internal_val = int(len(X_trainval_scaled) * 0.1)
indices = np.arange(len(X_trainval_scaled))
np.random.shuffle(indices)

internal_train_idx = indices[n_internal_val:]
internal_val_idx = indices[:n_internal_val]

internal_train_dataset = TensorDataset(
    torch.FloatTensor(X_trainval_scaled[internal_train_idx]),
    torch.FloatTensor(y_trainval_combined[internal_train_idx])
)
internal_val_dataset = TensorDataset(
    torch.FloatTensor(X_trainval_scaled[internal_val_idx]),
    torch.FloatTensor(y_trainval_combined[internal_val_idx])
)

internal_train_loader = DataLoader(internal_train_dataset, batch_size=best_params['batch_size'], shuffle=True)
internal_val_loader = DataLoader(internal_val_dataset, batch_size=best_params['batch_size'], shuffle=False)
...
final_model, _ = train_model(final_model, internal_train_loader, internal_val_loader, final_config, device)


# ============================================================================
# FINAL EVALUATION - FIRST TIME TOUCHING TEST SET!
# ============================================================================

print("\n" + "="*70)
print("🔒 UNLOCKING TEST SET FOR FIRST AND ONLY TIME 🔒")
print("="*70)
...
results = {
    ...
    'optuna_study': {
        'best_validation_f1': study.best_value,
...
print("\n" + "="*70)
print("🎯 DATA DISCIPLINE SUMMARY")
print("="*70)
print(f"✓ Optuna optimized on VALIDATION set ({len(X_val)} samples)")
print(f"✓ Test set was LOCKED during entire search ({len(X_test)} samples)")
print(f"✓ Final model trained on train+val ({len(X_train) + len(X_val)} samples)")
print(f"✓ Test set evaluated ONCE at the end: F1={test_f1:.4f}, AUC={test_auc:.4f}")
print("="*70)
